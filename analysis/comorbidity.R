...
for (i in 1:nrow(results)) {
  ...
  results[i,2] <- sum(df[,i] == 1 & df[,i+nrow(results)] == 0)/results[i,1]
  results[i,3] <- sum(df[,i] == 1 & df[,i+nrow(results)] == 1)/results[i,1]
  results[i,4] <- sum(df[,i] == 0 & df[,i+nrow(results)] == 1)/results[i,1]
}

# format percentages
library(scales)
results[,2:4] <- apply(results[,2:4], 2, percent, accuracy = 0.1)
