...
import delimited using "output/11c_an_int_tab_contents_HRtable_`outcome'_vaccine_child_prevacc.txt", clear
keep if age==`level'
...
scatter graphorder hr if int_level==1, mcol(green)	msize(small)		///										///
	...
	|| scatter graphorder disx2 , m(i) mlab(pval_new) mlabsize(small) mlabcol(black) 	///
		...
		text(-3 0.2 "Lower risk in those living with children", place(e) size(tiny)) ///
		text(-3 1.5 "Higher risk in those living with children", place(e) size(tiny))
		
graph export "output/an_vaccine_prevacc_`outcome'_age`level'.svg", as(svg) replace
sort obsorder
 list
	}
}
