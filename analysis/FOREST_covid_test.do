...
keep if outcome == "covid_tpp_prob" | outcome == "covid_test_ever"
gen outcome_new = 1 if outcome == "covid_tpp_prob"
replace outcome_new = 2 if outcome == "covid_test_ever"
lab define outcome_new 1 "Reported SARS-CoV-2 infection"  ///
2 "COVID-19 test ever"
...
gen hrtitle="Hazard Ratio (95% CI)" if graphorder == 43
...
scatter graphorder hr if period_text=="Period 1 - Schools closed", mcol(gs0)	msize(small)		///										///
	...
	|| scatter graphorder disx, m(i) mlab(bf_hrtitle) mlabsize(tiny) mlabcol(black) ///
		xline(1,lp(solid)) 															///
		xscale(log range(0.1 70)) xlab(0.5 1 2 4 8, labsize(vsmall)) xtitle("")  ///
		ylab(none) ytitle("")		yscale( lcolor(white))					///
		graphregion(color(white))  legend(off)  ysize(8) ///
		text(-2 0.01 "Lower risk in those living with children", place(e) size(tiny)) ///
		text(-2 2.0 "Higher risk in those living with children", place(e) size(tiny))
		
		
graph export "output/forest_covid_test_age`x'.svg", as(svg) replace
