...
file write tablecontents_all_outcomes ("period") _tab ("age") _tab ("exposure") _tab ("exposure level") ///
_tab ("outcome") _tab ("HR")  _tab ("lci")  _tab ("uci")  _n
foreach period in 0 1 2  {
forvalues x=0/1 {
foreach outcome in  covid_tpp_prob covidadmission covid_icu covid_death  {
forvalues i=1/3 {
local endwith "_tab"

	*put the varname and condition to left so that alignment can be checked vs shell
	file write tablecontents_all_outcomes ("`period'") _tab ("`x'") _tab ("`variable'") _tab ("`i'") _tab ("`outcome'") _tab
