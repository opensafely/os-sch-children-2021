...
import delimited using "output/11d_an_int_tab_contents_HRtable_`outcome'_vaccine_child_postvacc.txt", clear
keep if age==`level'
...
graph export "output/an_vaccine_postvacc_`outcome'_age`level'.svg", as(svg) replace
sort obsorder
 list
	}
}
